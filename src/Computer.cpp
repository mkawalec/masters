...
namespace turb {
    ...
    void Computer::fold(std::vector<std::vector<double> > *folded, int target_rank)
    {
        ...
        std::cout << rank << " here8" << std::endl;
        MPI_Wait(&send_request, &tmp_status);
        std::cout << rank << " here9" << std::endl;
        delete[] recv_array;
